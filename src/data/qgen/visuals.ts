...
export const VISUALS: Archetype[] = [
  ...
  },
  {
    ...
    gen: (r): RawQ => {
      ...
      return { diagram: { kind: "vgraph", graph: "vt", shape: charging ? "sine-up" : "sine-down", note: charging ? "V_C(t)" : "I(t)" }, prompt: `The graph shows a quantity in an RC circuit during ${charging ? "charging" : "discharging"}. Which physical quantity could the graph represent?`, choices: [charging ? "Capacitor voltage" : "Current", charging ? "Current" : "Capacitor charge", "Time constant", "Resistance of the circuit"], correct: 0, tempt: [undefined, charging ? "Charging current starts large and decays; it does not rise toward its final value." : "Capacitor charge does not decay if the circuit is charging.", "For fixed R and C, τ is a constant, not a time-varying curve.", "A component parameter is not the transient quantity shown."], explanation: charging ? "Capacitor voltage rises from zero toward the battery voltage with an exponential shape." : "During discharge, current magnitude decays exponentially toward zero.", equations: ["V_C=V_f(1-e^{-t/\\tau})", "I=I_0e^{-t/\\tau}"], commonMistake: "Confusing rising capacitor voltage with decaying charging current.", apStrategy: "Classify the graph by initial and final behavior before using the exponential equation.", category: "graph" };
    },
  },
  {
