...
export function PhysicsReferenceSheet({ initialCourse, compact = false }: { initialCourse?: CourseId; compact?: boolean }) {
  ...
  const [usedCourse, setUsedCourse] = useState<CourseId | undefined>(initialCourse);

  // Follow a changed initial course without re-render cascades.
  if (initialCourse !== usedCourse) {
    setUsedCourse(initialCourse);
